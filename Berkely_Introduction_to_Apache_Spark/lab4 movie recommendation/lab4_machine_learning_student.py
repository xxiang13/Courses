...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    return (IDandRatingsTuple[0],(numRating,avgRating))
...
movieIDsWithAvgRatingsRDD = movieIDsWithRatingsRDD.map(getCountsAndAverages)  #call udf, no need lambda funtion!
...
print 'movieNameWithAvgRatingsRDD: %s\n' % movieNameWithAvgRatingsRDD.take(1)
...
def computeError(predictedRDD, actualRDD):
    ...
    #give same key so can sum up all SE by using reduceByKey
    totalError = squaredErrorsRDD.map(lambda x: ('key',x[1])).reduceByKey(lambda x,y: x+y)

    # Count the number of entries for which you computed the total squared error
    numRatings = squaredErrorsRDD.count()

    # Using the total squared error and the number of entries, compute the RSME
    return math.sqrt(totalError.collect()[0][1]/float(numRatings))
...
bestIteration = -1
for rank in ranks:
    model = ALS.train(trainingRDD, rank, seed=seed, iterations=iterations,     #generate model
                      lambda_=regularizationParameter)
    predictedRatingsRDD = model.predictAll(validationForPredictRDD)            #predict
...
trainingAvgRating = trainingRDD.map(lambda x: ('key',x[2])).groupByKey().map(lambda x:(sum(x[1])/float(len(x[1]))))
print(trainingAvgRating.collect())[0]
...
trainingAvgRating = (trainingRDD
                     .map(lambda x: ('key',x[2]))    #get all rating into one tuple  by assgning same key 
                     .groupByKey()                   #and use groupByKey()
                     .map(lambda x:(sum(x[1])/float(len(x[1]))))
                     .collect()[0])      #comput average rating across all movies
...
myRatingsModel = ALS.train(trainingWithMyRatingsRDD, bestrank, seed=seed, iterations=iterations,     #generate model
                      lambda_=regularizationParameter)
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD))
print(ratingsWithNamesRDD.take(1))
